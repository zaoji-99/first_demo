...
class SimpleNet(nn.Module):
    ...
    def train_vis(self, vis, epoch, data_len, batch, loss, eid='main', name=None, win='vtrain'):
        ...
        # print(type([loss]), type(loss), type(loss.cpu()))
        # lloss = loss.cpu().numpy()
        # print(f'losstype: {type(loss)}')

        vis.line(X=np.array([(epoch-1)*data_len+batch]),
                                 Y=np.array([loss.detach().cpu().numpy()]),
                                 env=eid,
    ...
    def train_vis1(self, vis, epoch, data_len, batch, loss, eid='main', name=None, win='vtrain'):
